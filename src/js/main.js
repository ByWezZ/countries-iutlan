function fill_db() {
  ...
  countries.forEach(c => {
    ...
    if (c.currencies) {
      c.currencies.forEach(cu => {
        if (!(cu.code in Currency.all_currencies)) {
          ...
          Currency.all_currencies[currentCurrency.code] = currentCurrency;
        }
      });
    }
    ...
    if (currentCountry.alpha3Code) Country.all_countries[currentCountry.alpha3Code] = currentCountry;
    ...
    if (c.languages) {
      c.languages.forEach(lang => {
        if (!(lang.iso639_2 in Language.all_languages)) {
          ...
          Language.all_languages[currentLanguage.iso639_2] = currentLanguage;
        }
      });
    }
  });
